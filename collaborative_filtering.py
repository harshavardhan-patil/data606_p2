...
def run_cf_with_svd(ratings_df, k):
    ...
    # Split the data
    train_df, test_df  = split_data(cleaned_ratings)
    ...
    def evaluate_model(k_neighbors, n_components=20):
        """
    ...
    return evaluate_model(k_neighbors=k)

def evaluate_neighborhood_sizes(ratings_df):
    """Test different k values and return MAD scores"""
    k_values = [5, 10, 15, 20, 50, 100]
    results = {}
    
    for k in k_values:
        print(f"Testing k={k}")
        mad = run_cf_with_svd(ratings_df, k_neighbors=k)
        results[k] = mad
